...
interface PropertyCardsSliderProps {
    activeTab?: number;
    showAll?: boolean;
}
    ...
    ({ activeTab = 0, showAll = false }, ref) => {
        ...
        return (
            <>
                ...
                <div ref={ref} className="hidden lg:flex gap-6 overflow-x-auto hide-scrollbar px-4 pb-4">
                    {cards.map((item) => (
                        <Link
                            key={item.id}
                            href="/propertyDetails"
                            className="min-w-[280px] sm:min-w-[300px] md:min-w-[320px] bg-white rounded-2xl overflow-hidden block cursor-pointer flex-shrink-0"
                        >
                ...
                {/* Mobile/Tablet: Grid */}
                <div className="lg:hidden grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 gap-6 pb-4">
                    {(showAll ? cards : cards.slice(0, 4)).map((item) => (
                        <Link
                            key={item.id}
                            href="/propertyDetails"
...
PropertyCardsSlider.displayName = "PropertyCardsSlider";

export default PropertyCardsSlider;
